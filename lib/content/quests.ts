...
export const QUESTS: Quest[] = [
...

  {
    ...
    questions: [
      ...
      { id: 'conc4', text: 'A Python asyncio coroutine downloads a file and writes to a shared results list. You run 50 coroutines concurrently with asyncio.gather(). The list occasionally has fewer than 50 items. Why?', options: ['The GIL corrupts writes to lists', 'Coroutines that raise exceptions silently drop their result unless you await each or use return_exceptions=True', 'asyncio.gather() only runs 10 tasks in parallel', 'Lists are not thread-safe'], correctIndex: 1, explanation: 'By default asyncio.gather() suppresses exceptions from individual tasks — an exception in one coroutine means its result is never appended. Use return_exceptions=True or try/except inside each coroutine to handle failures.', difficulty: 'Medium', xpReward: 60, topic: 'Concurrency' },
...

  {
    ...
    questions: [
      ...
      { id: 'gi3', text: 'A real-time leaderboard must return the top score in under 1ms on every read, with hundreds of score updates per second. Reads far outnumber writes. Which data structure best fits the read requirement?', options: ['Sorted array (re-sorted on each update)', 'Max-heap (O(1) peek at maximum, O(log n) insert/delete)', 'Hash map (O(1) lookup by key)', 'Linked list sorted on insert'], correctIndex: 1, explanation: 'A max-heap stores the maximum at the root, giving O(1) reads. Each update is O(log n) via sift-up/sift-down. A sorted array requires O(n) shifts per update — far too slow for hundreds of updates per second.', difficulty: 'Medium', xpReward: 60, topic: 'General CS' },
